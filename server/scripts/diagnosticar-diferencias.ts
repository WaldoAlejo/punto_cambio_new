...
async function diagnosticarPunto(puntoNombre: string) {
  console.log(`\n${"=".repeat(80)}`);
  ...
  console.log(`\n1️⃣ SALDO INICIAL:`);
  console.log(
    `   Cantidad: $${saldoInicial ? Number(saldoInicial.cantidad_inicial) : 0}`
  );
  ...
  console.log(
    `   - No bancarios (incluidos): ${movimientosNoBancarios.length}`
  );
  ...
  for (const [tipo, data] of Object.entries(porTipo)) {
    console.log(
      `   ${tipo}: ${data.count} movimientos, Total: $${data.total.toFixed(2)}`
    );
  }
  ...
  for (const mov of movimientosNoBancarios) {
    ...
    if (index < 5 || index >= movimientosNoBancarios.length - 5) {
      console.log(
        `   ${tipo.padEnd(15)} $${monto
          .toFixed(2)
          .padStart(10)} → $${saldoAntes.toFixed(2)} → $${saldo.toFixed(2)}`
      );
    } else if (index === 5) {
      console.log(
        `   ... (${movimientosNoBancarios.length - 10} movimientos más) ...`
      );
    }
  }
  ...
  console.log(
    `   Saldo registrado: $${
      saldoRegistrado ? Number(saldoRegistrado.cantidad) : 0
    }`
  );
  console.log(
    `   Diferencia: $${(
      saldo - (saldoRegistrado ? Number(saldoRegistrado.cantidad) : 0)
    ).toFixed(2)}`
  );
  ...
  if (movimientosBancarios.length > 0) {
    ...
    for (const mov of movimientosBancarios) {
      console.log(
        `   ${mov.tipo_movimiento.padEnd(15)} $${Number(mov.monto)
          .toFixed(2)
