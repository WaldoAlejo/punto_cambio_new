 ...
 */
async function calcularSaldoScript(
 ...
 */
async function validarSignosMovimientos(): Promise<void> {
  ...
  for (const mov of movimientos) {
    ...
    if (tipo === "INGRESO" && monto < 0) {
      console.log(
        `❌ INGRESO con monto negativo: ${mov.puntoAtencion.nombre} - ${mov.moneda.codigo} - $${monto}`
      );
      errores++;
    } else if (tipo === "EGRESO" && monto > 0) {
      console.log(
        `❌ EGRESO con monto positivo: ${mov.puntoAtencion.nombre} - ${mov.moneda.codigo} - $${monto}`
      );
      errores++;
    }
  }
  ...
  if (errores === 0) {
    ...
    console.log(
      `❌ Se encontraron ${errores} movimientos con signos incorrectos\n`
    );
  }
}
 ...
 */
async function validarTiposMovimiento(): Promise<void> {
  ...
  for (const tipo of tiposEncontrados) {
    if (!tiposValidos.includes(tipo.tipo_movimiento)) {
      console.log(
        `⚠️ Tipo no estándar encontrado: "${tipo.tipo_movimiento}" (${tipo._count} movimientos)`
      );
      tiposInvalidos++;
    } else {
 ...
 */
async function compararSaldos(): Promise<ValidationResult[]> {
  ...
  for (const punto of puntos) {
    try {
      ...
      const saldoBackend = await saldoReconciliationService.calcularSaldoReal(
        punto.id,
        monedaUSD.id
      );
      ...
      console.log(
        `│ ${nombrePadded} │ ${backendPadded} │ ${scriptPadded} │ ${difPadded} │ ${estado}    │`
      );
    } catch (error) {
      console.log(
        `│ ${punto.nombre.padEnd(
 ...
 */
function generarResumen(resultados: ValidationResult[]): void {
  ...
  console.log(
    `✅ Perfectos (diferencia ≤ $0.02):  ${perfectos}/${resultados.length}`
  );
  console.log(
    `⚠️  Advertencias (diferencia ≤ $1): ${advertencias}/${resultados.length}`
  );
  console.log(
    `❌ Errores (diferencia > $1):       ${errores}/${resultados.length}\n`
  );
  ...
  if (perfectos === resultados.length) {
    ...
    console.log(
      "✅ El backend está funcionando correctamente con diferencias mínimas.\n"
    );
  } else {
    console.log(
      "⚠️ Se encontraron diferencias significativas. Revisar la lógica del backend.\n"
    );
  }
...
async function main() {
  ...
  if (USAR_RANGO_FECHAS) {
    ...
    console.log(
      `📅 Rango: ${FECHA_INICIO.toLocaleString("es-EC", {
