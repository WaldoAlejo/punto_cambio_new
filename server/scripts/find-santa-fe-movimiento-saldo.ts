...
async function findSantaFeMovimientoSaldo() {
  ...
  try {
    ...
    const movimientosPorDia = new Map<string, typeof movimientosSaldo>();
      ...
      movsDia.forEach((mov, index) => {
        ...
        console.log(
          `   ${(index + 1)
            ...
            2
          )} ${mov.tipo_movimiento.padEnd(25)} | ${
            mov.descripcion || "Sin descripción"
          } | Usuario: ${mov.usuario?.nombre || "N/A"} | Saldo: ${Number(
            mov.saldo_nuevo
          ).toFixed(2)}`
        );
      });
    ...
    if (movimientosSaldo.length <= 50) {
      ...
      movimientosSaldo.forEach((mov, index) => {
        ...
        console.log(
          `${(index + 1)
            .toString()
            .padStart(3)}. ${fecha} ${hora} | ${signo}${monto.toFixed(
            2
          )} | ${mov.tipo_movimiento.padEnd(25)} | Saldo: ${Number(
            mov.saldo_anterior
          ).toFixed(2)} → ${Number(mov.saldo_nuevo).toFixed(2)} | ${
            mov.descripcion || "Sin descripción"
          }`
        );
      });
    ...
    const saldoFinal = Number(
      movimientosSaldo[movimientosSaldo.length - 1]?.saldo_nuevo || 0
    );

    console.log(`💰 Saldo inicial: ${saldoInicial.toFixed(2)} USD`);
    console.log(`💳 Saldo final:   ${saldoFinal.toFixed(2)} USD`);
    console.log(
      `🔄 Cambio total:  ${(saldoFinal - saldoInicial).toFixed(2)} USD`
    );
  } catch (error) {
