...
/**
 * Script para generar un reporte de transferencias históricas mal contabilizadas
 * SIN hacer correcciones - solo para análisis
 */
...
interface TransferAnalysis {
  ...
  monto: number;
  via: string | null;
  tiene_movimiento_origen: boolean;
  tiene_movimiento_destino: boolean;
  problema: string;
}

async function analizarTransferencias(): Promise<TransferAnalysis[]> {
  ...
  const analisis: TransferAnalysis[] = [];
...
async function generarReporteInconsistencias() {
  ...
  if (inconsistencias.length > 0) {
    ...
    const totalDiferencia = inconsistencias.reduce(
      (sum, inc) => sum + Math.abs(inc.diferencia),
      0
    );
  ...
  try {
    ...
    if (problemasTransferencias.length > 0) {
      ...
      const porProblema = problemasTransferencias.reduce((acc, t) => {
        if (!acc[t.problema]) acc[t.problema] = [];
        acc[t.problema].push(t);
        return acc;
      }, {} as Record<string, TransferAnalysis[]>);
      ...
      Object.entries(porProblema).forEach(([problema, transferencias]) => {
        ...
        transferencias.slice(0, 10).forEach((t) => {
          ...
          logger.info(
            ...
            } | ${t.origen_nombre} → ${t.destino_nombre} | ${t.moneda_codigo} ${
              t.monto
            } | ${t.via || "N/A"}`
          );
        });
      ...
      logger.info("\n📊 RESUMEN:");
      logger.info(
        `   Total transferencias con problemas: ${problemasTransferencias.length}`
      );

      const montoTotalAfectado = problemasTransferencias.reduce(
        (sum, t) => sum + t.monto,
        0
      );
      ...
      const puntosAfectados = new Set(
        problemasTransferencias.map((t) => t.origen_nombre)
      ).size;
      logger.info(`   Puntos de origen afectados: ${puntosAfectados}`);
    } else {
      logger.info(
        "✅ Todas las transferencias están correctamente contabilizadas"
      );
    }
