...
router.get('/', authenticateToken, requireRole(['ADMIN', 'SUPER_USUARIO']), async (req: express.Request, res: express.Response): Promise<void> => {
  try {
    res.set({
...
router.patch('/:transferId/approve', authenticateToken, requireRole(['ADMIN', 'SUPER_USUARIO']), validate(approvalSchema), async (req: express.Request, res: express.Response): Promise<void> => {
  try {
    ...
    res.status(200).json({
      transfer: {
        ...
        fecha_aprobacion: updatedTransfer.fecha_aprobacion?.toISOString() || null,
...
router.patch('/:transferId/reject', authenticateToken, requireRole(['ADMIN', 'SUPER_USUARIO']), validate(approvalSchema), async (req: express.Request, res: express.Response): Promise<void> => {
  try {
    const { transferId } = req.params;
    const { observaciones } = req.body;
