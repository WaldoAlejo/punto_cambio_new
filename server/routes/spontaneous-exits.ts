...
const createExitSchema = z.object({
  motivo: z.enum(['BANCO', 'DILIGENCIA_PERSONAL', 'TRAMITE_GOBIERNO', 'EMERGENCIA_MEDICA', 'OTRO']),
...
router.post('/', authenticateToken, validate(createExitSchema), async (req: express.Request, res: express.Response): Promise<void> => {
  try {
...
router.patch('/:exitId/return', authenticateToken, validate(returnExitSchema), async (req: express.Request, res: express.Response): Promise<void> => {
  try {
    ...
    if (exit.usuario_id !== req.user?.id && !['ADMIN', 'SUPER_USUARIO'].includes(req.user?.rol || '')) {
      res.status(403).json({
    ...
    const duracionMinutos = Math.round((fechaRegreso.getTime() - exit.fecha_salida.getTime()) / (1000 * 60));
