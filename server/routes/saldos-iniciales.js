...
router.post('/', auth, async (req, res) => {
  try {
    ...
    `;
    const existingResult = await pool.query(existingQuery, [punto_atencion_id, moneda_id]);

    if (existingResult.rows.length > 0) {
      return res.status(400).json({
        success: false,
        error: 'Ya existe un saldo inicial activo para esta moneda en este punto'
      });
    }
    ...
    `;
    await pool.query(upsertSaldoQuery, [punto_atencion_id, moneda_id, parseFloat(cantidad_inicial)]);
