...
export const authenticateTokenDebug: RequestHandler = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  ...
  try {
    ...
    const effectiveUserId = decoded.id || decoded.userId;
    if (!effectiveUserId) {
      logger.error("DEBUG - No effective user ID found in token (post-verify)");
      res.status(401).json({
        error: "Token inválido - no user ID",
        success: false,
        timestamp: new Date().toISOString(),
    ...
    logger.info(
      "🚨 DEBUG - SALTANDO verificaciones de punto de atención para debugging",
      {
        userId: user.id,
        rol: user.rol,
        punto_atencion_id: user.punto_atencion_id,
