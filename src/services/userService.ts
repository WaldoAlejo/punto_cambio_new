...
export const userService = {
  async createUser(
    ...
    console.warn(
      "Input data JSON:",
      JSON.stringify({ ...data, password: "[HIDDEN]" }, null, 2)
    );
    ...
    try {
      ...
      return {
        user: null,
        error:
          err.response?.data?.error || "Error de conexión al crear usuario",
      };
    } finally {
  ...
  async getAllUsers(): Promise<{ users: Usuario[]; error: string | null }> {
    ...
    try {
      ...
      if (response.success) {
        console.warn(
          "getAllUsers - Success! Users count:",
      ...
      return {
        users: [],
        error:
          err.response?.data?.error || "Error de conexión al obtener usuarios",
      };
    } finally {
  ...
  async toggleUserStatus(
    ...
    try {
      ...
      if (response.success) {
        console.warn(
          "toggleUserStatus - Success! Updated user:",
          response.user
        );
        return { user: response.user, error: null };
      } else {
      ...
      return {
        ...
        error:
          err.response?.data?.error ||
          "Error de conexión al cambiar estado del usuario",
      };
    } finally {
