...
const Reports = ({ user, selectedPoint }: ReportsProps) => {
  const [reportType, setReportType] = useState<'exchanges' | 'transfers' | 'summary'>('summary');
  ...
  const generateReport = async () => {
    ...
    try {
      const filters = {
        dateFrom,
        dateTo,
        pointId: selectedPoint?.id || undefined,
  ...
  const exportReport = async () => {
    ...
    try {
      const data = reportType === 'exchanges' ? reportData.exchanges : reportData.transfers;
      await exportToExcel(data, `reporte_${reportType}_${dateFrom}_${dateTo}`);
    ...
    <div className="p-6 space-y-6">
      ...
      {reportData && (
        <div className="space-y-6">
          {reportType === 'summary' && (
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">{reportData.summary.totalExchanges}</div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">{reportData.summary.totalTransfers}</div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">${reportData.summary.totalVolume.toLocaleString()}</div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">${reportData.summary.averageTransaction.toLocaleString()}</div>
                </CardContent>
              </Card>
            </div>
          )}
          ...
          {reportType !== 'summary' && (
            <>
              <ReportChart data={reportType === 'exchanges' ? reportData.exchanges : reportData.transfers} />
              <ReportTable data={reportType === 'exchanges' ? reportData.exchanges : reportData.transfers} type={reportType} />
            </>
          )}
        </div>
      )}
      ...
      {!reportData && !isLoading && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Seleccione el tipo de reporte y el rango de fechas para generar el informe
            </CardDescription>
          </CardHeader>
          <CardContent>
