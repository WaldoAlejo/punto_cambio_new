...
## Seguimiento: permisos de consulta y validación por punto

Se reprodujo acceso de un OPERADOR a detalles de un cuadre ajeno mediante su identificador (200 en vez de 403). La revisión confirmó ausencia de comprobación de punto también en `/validar` y `/movimientos-auditoria`. Ahora detalles y validación consultan primero la cabecera para verificar el punto; auditoría valida el punto solicitado antes de leer movimientos. El operador solo accede a su punto asignado; los permisos administrativos y las listas de roles de cada ruta se conservan. Cuadre inexistente en detalles/validación responde 404.

**70 pruebas aprobadas**: acceso propio y rechazo ajeno del operador en las tres rutas, acceso entre puntos para ADMIN/SUPER_USUARIO, consultas de ADMINISTRATIVO y rechazo de su validación conforme a la regla existente. Los rechazos no contienen `data`. [Antes](INTEGRACION_LOCAL_PERMISOS_CUADRE_ANTES.json), [después](INTEGRACION_LOCAL_PERMISOS_CUADRE_CORREGIDOS.json). TypeScript correcto; ESLint de la ruta mantiene un error y una advertencia previos. Servicios locales detenidos, sin producción ni despliegue. Esto cubre esas tres rutas, no certifica los permisos de toda la aplicación.
