...
async function verificarConfiguracionServientrega(nombrePunto) {
    try {
        console.log(`🔍 Verificando configuración de Servientrega para el punto: ${nombrePunto}`);
        ...
        console.log(`🏪 Código de Agencia: ${punto.servientrega_agencia_codigo || "❌ NO CONFIGURADO"}`);
        console.log(`🏪 Nombre de Agencia: ${punto.servientrega_agencia_nombre || "❌ NO CONFIGURADO"}`);
        if (!tieneAgenciaCodigo) {
            console.log(`⚠️  PROBLEMA IDENTIFICADO: El punto no tiene código de agencia Servientrega configurado`);
            console.log(`   Esto impide que aparezca la opción "Guías Servientrega" en el menú del operador`);
        }
        console.log();
        ...
        if (punto.saldosServientrega) {
            ...
            console.log(`   💵 Saldo actual: $${Number(punto.saldosServientrega.monto_total || 0).toFixed(2)}`);
            console.log(`   📅 Creado en: ${punto.saldosServientrega.created_at.toLocaleString()}`);
        }
        else {
            console.log(`❌ No hay saldo de Servientrega configurado para este punto`);
        }
        console.log();
        ...
        if (!tieneAgenciaCodigo) {
            ...
            soluciones.push("✅ Configurar servientrega_agencia_codigo en la base de datos");
        }
        if (!tieneAgenciaNombre) {
            problemas.push("⚠️  Falta nombre de agencia Servientrega (opcional pero recomendado)");
            soluciones.push("✅ Configurar servientrega_agencia_nombre en la base de datos");
        }
        if (!punto.saldosServientrega) {
            problemas.push("❌ No hay saldo de Servientrega configurado");
            soluciones.push("✅ Asignar saldo inicial de Servientrega desde el panel de administración");
        }
        if (!punto.activo) {
        ...
        if (problemas.length === 0) {
            ...
            console.log('   La opción "Guías Servientrega" debería aparecer en el menú del operador.');
        }
        else {
        ...
        if (!tieneAgenciaCodigo) {
            ...
            console.log();
            console.log("⚠️  Reemplaza CODIGO_AGENCIA y NOMBRE_AGENCIA con los valores correctos");
        }
    }
    catch (error) {
