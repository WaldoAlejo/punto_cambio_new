...
async function verificarCierreTingo() {
    ...
    try {
        ...
        if (jornadaHoy) {
            ...
            const esValida = jornadaHoy.estado === "ACTIVO" || jornadaHoy.estado === "ALMUERZO";
            console.log(`🔍 ¿Estado válido para cierre? ${esValida ? "✅ SÍ" : "❌ NO"}`);
        }
        else {
        ...
        if (transaccionesHoy.length > 0) {
            transaccionesHoy.forEach((t, index) => {
                console.log(`${index + 1}. ${t.servicio} - ${t.moneda.simbolo}${t.monto} (${t.fecha}) - ${t.tipo_movimiento}`);
                if (t.descripcion) {
            ...
            const totalGeneral = transaccionesHoy.reduce((sum, t) => sum + Number(t.monto), 0);
        ...
        console.log(`🔍 ¿Es OPERADOR? ${operador.rol === "OPERADOR" ? "✅ SÍ" : "❌ NO"}`);
