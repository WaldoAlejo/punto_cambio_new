...
async function verificarJornadaTingo() {
    ...
    try {
        ...
        const esActiva = jornadaHoy.estado === "ACTIVO" || jornadaHoy.estado === "ALMUERZO";
        console.log(`\n🔍 ANÁLISIS DE ESTADO:`);
        console.log(`----------------------------------------`);
        console.log(`¿Estado es ACTIVO o ALMUERZO? ${esActiva ? "✅ SÍ" : "❌ NO"}`);
        console.log(`Estado actual: ${jornadaHoy.estado}`);
        if (!esActiva) {
            console.log(`\n⚠️ PROBLEMA IDENTIFICADO:`);
            console.log(`El estado de la jornada es "${jornadaHoy.estado}" pero la API busca estados "ACTIVO" o "ALMUERZO"`);
        ...
        if (activeScheduleAPI) {
            ...
            console.log(`Razón: No hay jornada con estado ACTIVO o ALMUERZO para hoy`);
        }
    }
    catch (error) {
