...
async function checkNegativeBalances() {
    ...
    for (const saldo of saldosNegativos) {
        console.log(`📍 ${saldo.puntoAtencion.nombre} - ${saldo.moneda.codigo}: ${saldo.cantidad}`);
    }
    ...
    if (saldoNegativo) {
        ...
        const saldoInicial = saldosIniciales.find((si) => si.moneda_id === saldoNegativo.moneda_id);
        if (saldoInicial) {
        ...
        for (const mov of movimientos) {
            ...
            console.log(`📝 ${mov.created_at.toISOString().split("T")[0]} | ${mov.tipo_movimiento} | ${operacion} | Acum: ${acumulado} | ${mov.descripcion || "Sin descripción"}`);
        }
