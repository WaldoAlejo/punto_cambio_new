...
async function limpiarCodigosServientrega() {
    try {
        ...
        console.log(`📍 Encontrados ${puntosConProblemas.length} puntos con configuración Servientrega`);
        console.log();
        ...
        for (const punto of puntosConProblemas) {
            ...
            const necesitaActualizacion = codigoOriginal !== codigoLimpio || nombreOriginal !== nombreLimpio;
        ...
        puntosActualizados.forEach((punto, index) => {
            ...
            const tieneEspacios = punto.servientrega_agencia_codigo?.includes(" ") ||
                punto.servientrega_agencia_nombre?.includes(" ");
            console.log(`   🔍 Estado: ${tieneEspacios ? "⚠️  Aún tiene espacios" : "✅ Limpio"}`);
            console.log();
        });
