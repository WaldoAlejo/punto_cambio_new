...
async function testSantaFeFix() {
    ...
    console.log(`📊 Saldo actual en BD: ${saldoActual?.cantidad || "No encontrado"}`);
    // Calcular saldo correcto con la lógica corregida
    const saldoCalculado = await saldoReconciliationService.calcularSaldoReal(santaFe.id, usd.id);
    ...
    if (Math.abs(diferencia) > 0.01) {
        ...
        const resultado = await saldoReconciliationService.reconciliarSaldo(santaFe.id, usd.id);
        if (resultado.corregido) {
            console.log(`✅ Saldo corregido de ${resultado.saldoAnterior} a ${resultado.saldoCalculado}`);
        }
        else {
