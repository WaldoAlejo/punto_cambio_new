...
async function contarRegistrosTabla(nombreTabla) {
    const result = await prisma.$queryRawUnsafe(`SELECT COUNT(*) as count FROM "${nombreTabla}"`);
...
async function migrarTabla(tabla) {
    ...
    try {
        ...
        for (const campo of tabla.camposFecha) {
            ...
            const campoExiste = await prisma.$queryRawUnsafe(`SELECT COUNT(*) as count FROM "${tabla.nombre}" WHERE "${campo}" IS NOT NULL`);
            ...
            console.log(`      ✅ ${registrosConFecha} registros con fecha en este campo`);
        }
...
async function verificarMigracion() {
    ...
    for (const ejemplo of ejemplos) {
        ...
        console.log(`────────────────────────────────────────────────────────`);
        try {
            const resultados = await prisma.$queryRawUnsafe(ejemplo.query);
...
async function main() {
    ...
    try {
        ...
        console.log("────────────────────────────────────────────────────────────────────────────────");
        console.log("   • PostgreSQL almacena fechas con timezone (timestamptz) en UTC internamente");
        console.log("   • Al configurar timezone = 'America/Guayaquil', PostgreSQL automáticamente:");
        console.log("     - Convierte fechas de entrada a UTC para almacenar");
        console.log("     - Convierte fechas de salida de UTC a hora local al consultar");
        console.log("   • Las fechas existentes en UTC se mostrarán automáticamente en hora Ecuador");
        console.log("   • Las nuevas fechas se guardarán en hora Ecuador (convertidas a UTC internamente)");
        console.log("────────────────────────────────────────────────────────────────────────────────\n");
        ...
        console.log("✅ Las nuevas fechas se guardarán automáticamente en hora local de Ecuador");
        console.log("\n📝 NOTA: El archivo server/lib/prisma.ts ya está configurado para usar");
        console.log("         timezone de Ecuador en todas las conexiones futuras.");
        console.log("════════════════════════════════════════════════════════════════════════════════\n");
    }
    catch (error) {
