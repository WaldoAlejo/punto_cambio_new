...
async function analyzeSantaFeMovements() {
    ...
    for (const mov of movimientos) {
        ...
        switch (mov.tipo_movimiento) {
            case "SALDO_INICIAL":
                ...
                console.log(`💰 ${fecha} | SALDO_INICIAL | +${monto} | Acum: ${saldoAcumulado.toFixed(2)}`);
                break;
            case "INGRESO":
                saldoAcumulado += monto;
                totalIngresos += monto;
                console.log(`📈 ${fecha} | INGRESO | +${monto} | Acum: ${saldoAcumulado.toFixed(2)} | ${mov.descripcion || "Sin descripción"}`);
                break;
            case "EGRESO":
                saldoAcumulado -= monto;
                totalEgresos += monto;
                console.log(`📉 ${fecha} | EGRESO | -${monto} | Acum: ${saldoAcumulado.toFixed(2)} | ${mov.descripcion || "Sin descripción"}`);
                break;
            case "TRANSFERENCIA_ENTRANTE":
                saldoAcumulado += monto;
                totalTransferenciasEntrantes += monto;
                console.log(`⬅️ ${fecha} | TRANSF_ENTRANTE | +${monto} | Acum: ${saldoAcumulado.toFixed(2)} | ${mov.descripcion || "Sin descripción"}`);
                break;
            case "TRANSFERENCIA_SALIENTE":
                saldoAcumulado -= monto;
                totalTransferenciasSalientes += monto;
                console.log(`➡️ ${fecha} | TRANSF_SALIENTE | -${monto} | Acum: ${saldoAcumulado.toFixed(2)} | ${mov.descripcion || "Sin descripción"}`);
                break;
            case "CAMBIO_DIVISA":
                if (mov.descripcion?.toLowerCase().includes("ingreso por cambio")) {
                    saldoAcumulado += monto;
                    totalCambiosIngreso += monto;
                    console.log(`🔄 ${fecha} | CAMBIO_DIVISA | +${monto} | Acum: ${saldoAcumulado.toFixed(2)} | ${mov.descripcion}`);
                }
                    ...
                    console.log(`🔄 ${fecha} | CAMBIO_DIVISA | ±${monto} | Acum: ${saldoAcumulado.toFixed(2)} | ${mov.descripcion} (NO PROCESADO)`);
                }
                break;
            case "AJUSTE":
                totalAjustes += monto;
                console.log(`⚙️ ${fecha} | AJUSTE | ${monto} | Acum: ${saldoAcumulado.toFixed(2)} | ${mov.descripcion || "Sin descripción"} (IGNORADO EN CÁLCULO)`);
                break;
            default:
                console.log(`❓ ${fecha} | ${mov.tipo_movimiento} | ${monto} | Acum: ${saldoAcumulado.toFixed(2)} | ${mov.descripcion || "Sin descripción"} (NO PROCESADO)`);
        }
    }
    console.log("\n📊 RESUMEN FINAL:\n");
    ...
    console.log(`⬅️ Transferencias entrantes: +${totalTransferenciasEntrantes.toFixed(2)}`);
    console.log(`➡️ Transferencias salientes: -${totalTransferenciasSalientes.toFixed(2)}`);
    ...
    const totalEntradas = totalSaldoInicial +
        totalIngresos +
        totalTransferenciasEntrantes +
        totalCambiosIngreso;
    const totalSalidas = totalEgresos + totalTransferenciasSalientes + totalCambiosEgreso;
    console.log(`\n🟢 Total entradas: ${totalEntradas.toFixed(2)}`);
    console.log(`🔴 Total salidas: ${totalSalidas.toFixed(2)}`);
    console.log(`📊 Saldo calculado: ${(totalEntradas - totalSalidas).toFixed(2)}`);
    ...
    if (saldoAcumulado < 0) {
        ...
        console.log(`❌ El punto SANTA FE tiene más salidas (${totalSalidas.toFixed(2)}) que entradas (${totalEntradas.toFixed(2)})`);
        ...
        console.log("2. Transferencias salientes sin transferencias entrantes de compensación");
