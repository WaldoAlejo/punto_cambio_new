...
async function main() {
    ...
    console.log(`📅 Fecha de corte: ${formatEcuadorDateTime(FECHA_CORTE)} (Ecuador)`);
    console.log("═".repeat(100) + "\n");
    ...
    console.log("─".repeat(100));
    console.log("Punto de Atención".padEnd(35) +
    ...
    for (const punto of puntos) {
        ...
        const saldoActualStr = saldoActual !== null ? `$${saldoActual.toFixed(2)}` : "Sin saldo";
        const diferenciaStr = diferencia !== null
            ? diferencia >= 0
                ? `+$${diferencia.toFixed(2)}`
                : `-$${Math.abs(diferencia).toFixed(2)}`
            : "N/A";
        console.log(punto.nombre.padEnd(35) +
    ...
    console.log("─".repeat(100) + "\n");
    // Pedir confirmación
    const confirmar = await preguntarConfirmacion("⚠️  ¿Deseas actualizar la tabla Saldo con estos valores?");
    if (!confirmar) {
