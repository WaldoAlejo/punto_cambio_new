    ...
    try {
        ...
        for (const [fecha, movsDia] of Array.from(movimientosPorDia.entries()).sort()) {
            ...
            movsDia.forEach((mov, index) => {
                ...
                console.log(`   ${(index + 1)
                    .toString()
                    .padStart(2)}. [${hora}] ${signo}${monto.toFixed(2)} ${mov.tipo.padEnd(20)} | ${mov.descripcion || "Sin descripción"} | Usuario: ${mov.usuario?.nombre || "N/A"} | Saldo: ${saldoAcumulado.toFixed(2)}`);
            });
            console.log(`   ─────────────────────────────────────────────────────────────`);
            ...
            console.log(`   📊 Movimiento neto:  ${(ingresosDia - egresosDia).toFixed(2)} USD`);
        }
        console.log("\n" + "=".repeat(80));
