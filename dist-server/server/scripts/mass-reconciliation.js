...
async function ejecutarReconciliacionMasiva() {
    ...
    logger.info(`👤 Usuario administrador encontrado: ${adminUser.nombre} (${adminUser.id})`);
    console.log(`👤 Usuario administrador encontrado: ${adminUser.nombre} (${adminUser.id})`);
    ...
    logger.info(`📍 Encontrados ${puntos.length} puntos de atención para reconciliar`);
    console.log(`📍 Encontrados ${puntos.length} puntos de atención para reconciliar`);
    ...
    for (const punto of puntos) {
        try {
            ...
            logger.info(`✅ Punto ${punto.nombre}: ${corregidos}/${resultados.length} saldos corregidos`);
            console.log(`✅ Punto ${punto.nombre}: ${corregidos}/${resultados.length} saldos corregidos`);
            // Mostrar detalles de las correcciones
            for (let i = 0; i < resultados.length; i++) {
                const resultado = resultados[i];
                if (resultado.corregido && saldos[i]) {
                    const monedaCodigo = saldos[i].moneda.codigo;
                    logger.info(`   💰 ${monedaCodigo}: ${resultado.saldoAnterior} → ${resultado.saldoCalculado} (diferencia: ${resultado.diferencia})`);
                    console.log(`   💰 ${monedaCodigo}: ${resultado.saldoAnterior} → ${resultado.saldoCalculado} (diferencia: ${resultado.diferencia})`);
                }
            }
        }
        catch (error) {
