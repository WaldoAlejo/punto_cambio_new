...
async function analizarTransferenciasHistoricas() {
    ...
    logger.info(`📊 Encontradas ${transferencias.length} transferencias con origen`);
    const transferenciasIncompletas = [];
    ...
    logger.info(`⚠️ Encontradas ${transferenciasIncompletas.length} transferencias incompletas`);
    return transferenciasIncompletas;
}
async function corregirTransferenciaIncompleta(transferencia) {
    if (!transferencia.origen_id)
        return;
    logger.info(`🔧 Corrigiendo transferencia ${transferencia.numero_recibo || "N/A"}`, {
    ...
    try {
        ...
        logger.info(`✅ Movimiento de EGRESO creado para transferencia ${transferencia.numero_recibo || "N/A"}`);
    }
    catch (error) {
        logger.error(`❌ Error corrigiendo transferencia ${transferencia.numero_recibo || "N/A"}`, {
...
async function ejecutarReconciliacionMasiva() {
    ...
    for (const punto of puntos) {
        try {
            ...
            if (corregidos > 0) {
                logger.info(`✅ Punto ${punto.nombre}: ${corregidos} saldos corregidos`);
            }
        }
        catch (error) {
    ...
    logger.info(`🎯 Reconciliación masiva completada: ${totalCorregidos} saldos corregidos en total`);
}
async function generarReporteAntes() {
    logger.info("📊 Generando reporte de inconsistencias ANTES de la corrección...");
    ...
    if (inconsistencias.length > 0) {
        ...
        inconsistencias.forEach((inc) => {
            logger.info(`  - ${inc.puntoNombre} (${inc.monedaCodigo}): Registrado ${inc.saldoRegistrado}, Real ${inc.saldoCalculado}, Diferencia ${inc.diferencia}`);
        });
    }
    else {
...
async function generarReporteDespues() {
    logger.info("📊 Generando reporte de inconsistencias DESPUÉS de la corrección...");
    const inconsistencias = await saldoReconciliationService.generarReporteInconsistencias();
    if (inconsistencias.length > 0) {
        logger.warn("⚠️ INCONSISTENCIAS RESTANTES DESPUÉS DE LA CORRECCIÓN:");
        inconsistencias.forEach((inc) => {
            logger.warn(`  - ${inc.puntoNombre} (${inc.monedaCodigo}): Registrado ${inc.saldoRegistrado}, Real ${inc.saldoCalculado}, Diferencia ${inc.diferencia}`);
        });
    }
    else {
...
async function main() {
    try {
        ...
        if (transferenciasIncompletas.length === 0) {
            logger.info("✅ No se encontraron transferencias incompletas. El sistema ya está correcto.");
            return;
        }
        // 3. Mostrar resumen de lo que se va a corregir
        logger.info("📋 RESUMEN DE CORRECCIONES A REALIZAR:");
        logger.info(`  - ${transferenciasIncompletas.length} transferencias incompletas encontradas`);
        const montoTotal = transferenciasIncompletas.reduce((sum, t) => sum + t.monto, 0);
        logger.info(`  - Monto total afectado: ${montoTotal.toFixed(2)}`);
        const puntosAfectados = new Set(transferenciasIncompletas.map((t) => t.origen_id)).size;
        ...
        for (let i = 0; i < transferenciasIncompletas.length; i++) {
            ...
            logger.info(`[${i + 1}/${transferenciasIncompletas.length}] Corrigiendo ${transferencia.numero_recibo || "N/A"}...`);
        ...
        logger.error("💥 Error durante la corrección de transferencias históricas", {
            error: error instanceof Error ? error.message : "Unknown error",
