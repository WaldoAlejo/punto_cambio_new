...
router.post("/", authenticateToken, async (req, res) => {
    try {
        const { detalles = [], observaciones, tipo_cierre = "CERRADO", allowMismatch = false, } = req.body || {};
        ...
        const totalIngresos = detalles.reduce((sum, d) => sum + asNumber(d.ingresos_periodo), 0);
        const totalEgresos = detalles.reduce((sum, d) => sum + asNumber(d.egresos_periodo), 0);
        const totalMovimientos = detalles.reduce((sum, d) => sum + asNumber(d.movimientos_periodo), 0);
        ...
        if (detalles.length > 0 && !allowMismatch) {
            ...
            if (diferenciasInvalidas.length > 0) {
                return res.status(400).json({
                    success: false,
                    error: "Las diferencias superan la tolerancia permitida. Si es correcto, habilita 'allowMismatch' para continuar.",
