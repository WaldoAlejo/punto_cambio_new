...
router.post("/procesar-cambio", authenticateToken, saldoValidation.validarSaldoSuficiente, async (req, res) => {
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (!cambio) {
                throw new Error("Cambio no encontrado para procesar movimientos contables");
            }
            ...
            for (const mov of movimientos) {
                ...
                const monedasActual = new Prisma.Decimal(saldoActual?.monedas_fisicas ?? 0);
                const deltaCantidad = tipo_movimiento === "INGRESO" ? montoNum : -montoNum;
                ...
                if (moneda_id === cambio.moneda_destino_id) {
                    ...
                    deltaMonedasFisicas = -Number(cambio.divisas_recibidas_monedas || 0);
                }
                    ...
                    deltaMonedasFisicas = Number(cambio.divisas_entregadas_monedas || 0);
                }
                // Nuevos valores calculados
                const nuevaCantidad = cantidadActual.add(new Prisma.Decimal(deltaCantidad));
                const nuevosBilletes = billetesActual.add(new Prisma.Decimal(deltaBilletes));
                const nuevasMonedas = monedasActual.add(new Prisma.Decimal(deltaMonedasFisicas));
                // Validaciones: evitar negativos
                if (tipo_movimiento === "EGRESO") {
                    if (nuevaCantidad.lessThan(0)) {
                        throw new Error(`Saldo insuficiente. Disponible: ${cantidadActual.toFixed(2)}, Requerido: ${montoNum.toFixed(2)}`);
                    }
