...
const toNum = (x) => Number((x ?? d0()).toString());
/* ============================================================================
...
router.get("/", authenticateToken, async (req, res) => {
    try {
        ...
        const [cambiosDivisas, serviciosExternos, transferencias, puntosActivos, monedasActivas,] = await Promise.all([
        ...
        for (const moneda of monedas) {
            ...
            cambiosOrigenAgg, cambiosDestinoAgg, 
            // Servicios externos: ingresos/egresos
            serviciosIngresosAgg, serviciosEgresosAgg, 
            ...
            const cambiosDivisasDestino = cambiosDestinoAgg._sum.monto_destino ?? d0();
            ...
            const transferIn = sumDestAgg._sum?.monto ?? d0();
            const transferOut = sumOrigAgg._sum?.monto ?? d0();
            ...
            const hasMovement = !balance.equals(0) ||
        ...
        console.log(`✅ Balance calculado: ${balancesRaw.length} monedas con movimientos`);
        res.json({
            success: true,
            data: {
                resumenGeneral,
                balancesPorMoneda: balancesRaw.map((b) => ({
                    ...
                    detalles: {
                        ...
                        serviciosExternosIngresos: toNum(b.detalles.serviciosExternosIngresos),
                        serviciosExternosEgresos: -toNum(b.detalles.serviciosExternosEgresos),
...
router.get("/punto/:pointId", authenticateToken, async (req, res) => {
    try {
        ...
        const [cambiosDivisas, serviciosExternos, transferenciasOrigen, transferenciasDestino,] = await Promise.all([
        ...
        const balancePorPunto = {
            ...
            totalMovimientos: cambiosDivisas +
                serviciosExternos +
                transferenciasOrigen +
                transferenciasDestino,
        };
        console.log(`✅ Balance por punto calculado: ${balancePorPunto.totalMovimientos} movimientos totales`);
        res.json({
            success: true,
            data: {
                balancePorPunto,
