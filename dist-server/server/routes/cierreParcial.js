...
router.post("/parcial", authenticateToken, async (req, res) => {
    try {
        const { detalles = [], observaciones, allowMismatch = false, } = req.body || {};
        ...
        // Totales
        const totalIngresos = detalles.reduce((s, d) => s + asNumber(d.ingresos_periodo), 0);
        const totalEgresos = detalles.reduce((s, d) => s + asNumber(d.egresos_periodo), 0);
        const totalMovimientos = detalles.reduce((s, d) => s + asNumber(d.movimientos_periodo), 0);
