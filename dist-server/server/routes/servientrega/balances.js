...
router.get("/saldo/validar/:puntoAtencionId", async (req, res) => {
    try {
        ...
        console.log(`🔍 Servientrega: Validando saldo para punto ${puntoAtencionId}, monto: ${monto}`);
        if (!puntoAtencionId) {
        ...
        if (!saldo) {
            console.log(`❌ Servientrega: No se encontró saldo para punto ${puntoAtencionId}`);
            return res.json({
                estado: "SIN_SALDO",
        ...
        const montoRequerido = monto ? parseFloat(monto) : 0;
        console.log(`💰 Servientrega: Saldo disponible: ${disponible}, Monto requerido: ${montoRequerido}`);
        if (disponible <= 0) {
            return res.json({
        ...
        if (montoRequerido > 0 && disponible < montoRequerido) {
            return res.json({
                ...
                mensaje: `Saldo insuficiente. Disponible: $${disponible.toFixed(2)}, Requerido: $${montoRequerido.toFixed(2)}`,
                disponible: disponible,
                monto_requerido: montoRequerido,
            });
        }
        ...
        console.error(`❌ Servientrega: Error al validar saldo para punto ${req.params.puntoAtencionId}:`, error);
        res.status(500).json({
            estado: "ERROR",
...
router.get("/saldo/:puntoAtencionId", async (req, res) => {
    try {
        const { puntoAtencionId } = req.params;
        console.log(`💰 Servientrega: Consultando saldo para punto ${puntoAtencionId}`);
        if (!puntoAtencionId) {
            console.error("❌ Servientrega: ID de punto de atención no proporcionado");
            return res
                .status(400)
        ...
        const saldo = await dbService.obtenerSaldo(puntoAtencionId);
        if (!saldo) {
            console.log(`💰 Servientrega: No se encontró saldo para punto ${puntoAtencionId}, devolviendo 0`);
        ...
        console.log(`✅ Servientrega: Saldo para punto ${puntoAtencionId}:`, resultado);
        res.json(resultado);
    }
    catch (error) {
        console.error(`❌ Servientrega: Error al obtener saldo para punto ${req.params.puntoAtencionId}:`, error);
        res.status(500).json({
...
router.post("/solicitar-saldo", async (req, res) => {
    try {
        const { punto_atencion_id, monto_solicitado, observaciones, creado_por } = req.body;
        // Validaciones
        if (!punto_atencion_id) {
            return res
                .status(400)
...
router.put("/solicitar-saldo/:id/estado", async (req, res) => {
    try {
        ...
        const dbService = new ServientregaDBService();
        const solicitud = await dbService.actualizarEstadoSolicitudSaldo(id, estado, aprobado_por);
        res.json({
            success: true,
            solicitud,
