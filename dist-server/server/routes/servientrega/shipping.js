...
function getCredentialsFromEnv() {
    ...
    if (!usuingreso || !contrasenha) {
        throw new Error("Faltan SERVIENTREGA_USER y/o SERVIENTREGA_PASSWORD en el entorno (.env.production).");
    }
...
router.post("/tarifa", async (req, res) => {
    try {
        ...
        const isInternacional = (paisDes && paisDes !== "ECUADOR") || (paisOri && paisOri !== "ECUADOR");
        const bodyConTipo = {
            ...req.body,
        ...
        const validationErrors = ServientregaValidationService.validateTarifaRequest(bodyConTipo);
        ...
        const sanitizedData = ServientregaValidationService.sanitizeTarifaRequest(bodyConTipo);
        // DEBUG: Log del payload que se va a enviar
        console.log("🔍 PAYLOAD ORIGINAL DEL FRONTEND:", JSON.stringify(req.body, null, 2));
        console.log("🔍 PAYLOAD CON TIPO:", JSON.stringify(bodyConTipo, null, 2));
        console.log("🔍 PAYLOAD SANITIZADO:", JSON.stringify(sanitizedData, null, 2));
...
router.post("/generar-guia", async (req, res) => {
    try {
        ...
        const retiroEsSi = retiroOficinaValor.toUpperCase() === "SI" ||
            req.body?.retiro_oficina === true;
        if (retiroEsSi && !req.body?.nombre_agencia_retiro_oficina) {
            return res.status(400).json({
                error: "Validación",
                mensaje: "nombre_agencia_retiro_oficina es requerido cuando retiro_oficina = 'SI'",
            });
        }
        // Construcción robusta del payload si NO viene formateado
        let payload;
        if (!yaFormateado) {
            const { remitente, destinatario, nombre_producto, contenido, retiro_oficina, nombre_agencia_retiro_oficina, pedido, factura, medidas, 
            // opcionales auxiliares
            punto_atencion_id, valor_total, } = req.body || {};
            // Normalizar producto
            const productoUpper = String(nombre_producto || "").toUpperCase();
            const producto = productoUpper.includes("DOC") || productoUpper === "DOCUMENTO"
                ? "DOCUMENTO"
                : "MERCANCIA PREMIER";
            // Formato "CIUDAD-PROVINCIA" en mayúsculas
            const ciudadOrigen = `${String(remitente?.ciudad || "").toUpperCase()}-${String(remitente?.provincia || "").toUpperCase()}`;
            const ciudadDestino = `${String(destinatario?.ciudad || "").toUpperCase()}-${String(destinatario?.provincia || "").toUpperCase()}`;
            ...
            const piezas = Number(medidas?.piezas ?? 1) || 1;
            // Cálculo de peso volumétrico (si hay dimensiones)
            const peso_volumentrico = alto > 0 && ancho > 0 && largo > 0 ? (alto * ancho * largo) / 5000 : 0;
            payload = {
                tipo: "GeneracionGuia",
                ...
                cedula_destinatario: destinatario?.identificacion || destinatario?.cedula || "",
        ...
        if (guia && base64) {
            ...
            const valorTotal = Number(processed?.total_transacion) ||
            ...
            try {
                ...
                if (!String(req.body?.tipo || "").includes("GeneracionGuia")) {
                    ...
                    if (req.body?.punto_atencion_id && valorTotal > 0) {
                        await db.descontarSaldo(req.body.punto_atencion_id, Number(valorTotal));
                    }
                }
            }
            catch (dbErr) {
...
router.get("/guias", async (req, res) => {
    try {
        ...
        const dbService = new ServientregaDBService();
        const guias = await dbService.obtenerGuias(desde || undefined, hasta || undefined);
