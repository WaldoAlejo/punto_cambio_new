...
import { TipoRecibo } from "@prisma/client";
const router = Router();
// Usaremos MOVIMIENTO para los recibos de Servientrega
const RECIBO_TIPO_SERVIENTREGA = TipoRecibo.MOVIMIENTO;
// ====================================
// 📄 CREAR RECIBO DE SERVIENTREGA
// ====================================
router.post("/", authenticateToken, async (req, res) => {
    try {
        const { numero_recibo, referencia_id, // ID de la guía de Servientrega
        punto_atencion_id, datos_operacion, // Datos completos de la operación (guía + tarifa)
        numero_copias = 2, } = req.body;
        ...
        if (!numero_recibo ||
            ...
            return res.status(400).json({
                error: "Faltan campos requeridos: numero_recibo, referencia_id, punto_atencion_id, datos_operacion",
            });
        }
        // Normaliza datos_operacion a InputJsonValue (evita TS2322)
        const datosOperacion = datos_operacion ??
            {};
...
// ====================================
router.get("/guia/:numeroGuia", authenticateToken, async (req, res) => {
    try {
...
// ====================================
router.patch("/:id/impreso", authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const { impreso = true, numero_copias } = req.body;
        const updateData = { impreso: Boolean(impreso) };
        ...
        logger?.info?.(`Servientrega: Recibo ${recibo.numero_recibo} marcado como ${updateData.impreso ? "impreso" : "no impreso"}`);
        res.json({
            success: true,
            message: `Recibo marcado como ${updateData.impreso ? "impreso" : "no impreso"}`,
        });
    }
    catch (error) {
...
router.get("/", authenticateToken, async (req, res) => {
    try {
        const { punto_atencion_id, fecha_desde, fecha_hasta, impreso, limit = "50", offset = "0", } = req.query;
        const whereClause = { tipo_operacion: RECIBO_TIPO_SERVIENTREGA };
        if (punto_atencion_id)
            whereClause.punto_atencion_id = String(punto_atencion_id);
        if (fecha_desde || fecha_hasta) {
            whereClause.fecha = {};
            if (fecha_desde)
                whereClause.fecha.gte = new Date(String(fecha_desde));
            if (fecha_hasta)
                whereClause.fecha.lte = new Date(String(fecha_hasta));
        }
        if (impreso !== undefined)
            whereClause.impreso = String(impreso) === "true";
        ...
        // Batch para nombres
        const puntoIds = Array.from(new Set(recibos.map((r) => r.punto_atencion_id).filter(Boolean)));
        const usuarioIds = Array.from(new Set(recibos.map((r) => r.usuario_id).filter(Boolean)));
        ...
        const mapUsuarios = new Map(usuarios.map((u) => [u.id, u.nombre || u.username || ""]));
        res.json({
            success: true,
