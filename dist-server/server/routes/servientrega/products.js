...
function normalizarProducto(raw) {
    ...
    if (c.includes("MERCANCIA") && c.includes("PREMIER"))
        return MERC;
...
router.post("/productos", async (_req, res) => {
    try {
        ...
        const rawList = (Array.isArray(result?.fetch) && result.fetch) ||
            (Array.isArray(result?.productos) && result.productos) ||
            (Array.isArray(result) && result) ||
            [];
        const normalizados = uniquePreserveOrder(rawList
            .map((item) => normalizarProducto(
        // soporta distintas formas:
        item?.producto ?? item?.nombre_producto ?? item))
            .filter((x) => x === DOC || x === MERC));
        const productosFinales = normalizados.length > 0 ? normalizados : [MERC, DOC]; // fallback seguro
        return res.json({
            success: true,
