...
function esAnulacionExitosa(resp) {
    ...
    if (typeof resp === "string") {
        ...
        if (match?.[1] && /actualizad/i.test(match[1]))
            return true;
    }
    return false;
}
...
function getCreds() {
    ...
    if (!usuingreso || !contrasenha) {
        throw new Error("Faltan variables de entorno SERVIENTREGA_USER y/o SERVIENTREGA_PASSWORD. Verifica .env.production");
    }
...
async function procesarAnulacionServientrega(numeroGuia) {
    ...
    if (!esAnulacionExitosa(response)) {
        throw new Error(`Respuesta inesperada de Servientrega: ${JSON.stringify(response)}`);
    }
    return response;
}
/* ==============================
...
============================== */
router.put("/solicitudes-anulacion/:id/responder", authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const estado = String(accion).toUpperCase() === "APROBAR" ? "APROBADA" : "RECHAZADA";
        ...
        if (estado === "APROBADA") {
            try {
                ...
                await dbService.actualizarSolicitudAnulacion(id, {
                    observaciones_respuesta: `${observaciones || ""}\n\nError en API Servientrega: ${apiError instanceof Error ? apiError.message : String(apiError)}`,
                });
            }
        }
        return res.json({
...
============================== */
router.post("/responder-solicitud-anulacion", authenticateToken, async (req, res) => {
    try {
        ...
        if (!solicitud_id || !validarAccion(accion)) {
            return res.status(400).json({
                success: false,
                error: "Parámetros inválidos",
                message: 'solicitud_id es requerido y la acción debe ser "APROBAR" o "RECHAZAR".',
            });
        }
        const estado = String(accion).toUpperCase() === "APROBAR" ? "APROBADA" : "RECHAZADA";
        ...
        console.error("❌ Error al responder solicitud de anulación (alias):", error);
        return sendError(res, 500, error);
    }
});
