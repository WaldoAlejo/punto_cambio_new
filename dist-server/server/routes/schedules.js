...
import { z } from "zod";
import { gyeDayRangeUtcFromDate, gyeDayRangeUtcFromYMD, gyeParseDateOnly, } from "../utils/timezone.js";
...
router.get("/", authenticateToken, async (req, res) => {
    try {
        ...
        if (req.query.usuario_id &&
            ...
            whereClause.usuario_id = req.query.usuario_id;
        }
        ...
        if (String(req.query.format || "").toLowerCase() === "csv") {
            ...
            const csv = [header, ...rows]
                .map((cols) => cols
                .map((c) => typeof c === "string" && (c.includes(",") || c.includes("\n"))
                ? `"${c.replaceAll('"', '""')}"`
                : String(c))
                .join(","))
                .join("\n");
            res.setHeader("Content-Type", "text/csv; charset=utf-8");
            res.setHeader("Content-Disposition", `attachment; filename="jornadas_${new Date()
                .toISOString()
                .slice(0, 10)}.csv"`);
...
// ==============================
router.post("/", authenticateToken, validate(scheduleSchema), async (req, res) => {
    try {
        const { usuario_id, punto_atencion_id, fecha_inicio, fecha_almuerzo, fecha_regreso, fecha_salida, ubicacion_inicio, ubicacion_salida, override, } = req.body;
        ...
        if ((req.user?.rol === "OPERADOR" ||
            ...
            res.status(403).json({
                error: "Los operadores y administrativos solo pueden gestionar sus propias jornadas",
                success: false,
                timestamp: new Date().toISOString(),
            });
            return;
        ...
        const esPrivilegiado = rol === "ADMINISTRATIVO" || rol === "ADMIN" || rol === "SUPER_USUARIO";
        ...
        if (!existingSchedule && !esPrivilegiado) {
            ...
            if (puntoOcupado) {
                res.status(409).json({
                    success: false,
                    error: "Este punto ya tiene una jornada activa. Selecciona otro punto o espera a que se libere.",
                    timestamp: new Date().toISOString(),
                });
                return;
            }
        ...
        if (existingSchedule) {
            ...
            if (fecha_salida) {
                ...
                console.log(`🚀 FINALIZACION_JORNADA_INICIADA - Usuario: ${req.user?.rol}, Punto: ${punto_atencion_id}`);
                if (!esExentoDeCaja(req.user?.rol)) {
                    ...
                    console.log(`🔍 BUSCANDO_CIERRE - Punto: ${punto_atencion_id}, Fecha: ${gte.toISOString()}`);
                    ...
                    console.log(`📊 RESULTADO_CIERRE - Encontrado: ${!!cierreHoy}, ID: ${cierreHoy?.id || "N/A"}`);
                    if (!cierreHoy) {
                        console.log(`❌ ERROR_CIERRE_REQUERIDO - Enviando respuesta de error`);
                        res.status(400).json({
                            success: false,
                            error: "Debe realizar el cierre de caja diario antes de finalizar su jornada",
                        });
                        return;
                    }
                    ...
                    if (!cierreSE) {
                        res.status(400).json({
                            success: false,
                            error: "Debe realizar el cierre diario de Servicios Externos antes de finalizar su jornada",
                        });
                        return;
                    }
                }
...
router.get("/user/:id", authenticateToken, async (req, res) => {
    try {
        ...
        const { from, to, estados } = req.query;
...
router.post("/reassign-point", authenticateToken, validate(reassignSchema), async (req, res) => {
    try {
        ...
        const { usuario_id, destino_punto_atencion_id, motivo, observaciones, finalizar, } = req.body;
        ...
        if (!newPointId) {
            ...
            if (!principal) {
                res.status(400).json({
                    success: false,
                    error: "No se encontró punto principal. Especifique destino_punto_atencion_id",
                });
                return;
            }
