 ...
 */
router.post("/reconcile-point/:pointId", authenticateToken, async (req, res) => {
    try {
        // Validar parámetros
        ...
        const resultados = await saldoReconciliationService.reconciliarTodosPuntoAtencion(pointId, usuarioId);
 ...
 */
router.post("/reconcile-single/:pointId/:currencyId", authenticateToken, async (req, res) => {
    try {
        // Validar parámetros
        ...
        // Ejecutar reconciliación
        const resultado = await saldoReconciliationService.reconciliarSaldo(pointId, currencyId, usuarioId);
        res.status(200).json({
            success: true,
...
router.post("/fix-all", authenticateToken, async (req, res) => {
    try {
        ...
        for (const inc of inconsistencias) {
            try {
                const resultado = await saldoReconciliationService.reconciliarSaldo(inc.puntoAtencionId, inc.monedaId, req.user?.id);
        ...
        logger.warn(`✅ Corrección masiva completada: ${corregidos} saldos corregidos`, {
            usuarioId: req.user?.id,
            totalInconsistencias: inconsistencias.length,
