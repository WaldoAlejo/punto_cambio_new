...
router.post("/", authenticateToken, validate(createExitSchema), async (req, res) => {
    try {
...
router.patch("/:exitId/return", authenticateToken, validate(returnExitSchema), async (req, res) => {
    try {
        ...
        if (exit.usuario_id !== req.user?.id &&
            !["ADMIN", "SUPER_USUARIO"].includes(req.user?.rol || "")) {
            res.status(403).json({
