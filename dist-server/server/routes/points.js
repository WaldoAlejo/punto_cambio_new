...
function formatPoint(punto) {
    return {
        ...
        servientrega_agencia_codigo: punto.servientrega_agencia_codigo ?? null,
        servientrega_agencia_nombre: punto.servientrega_agencia_nombre ?? null,
        activo: punto.activo,
...
router.get("/", authenticateToken, async (req, res) => {
    try {
        ...
        const formatted = puntos.map((p) => formatPoint(p));
...
router.get("/all", authenticateToken, requireRole(["ADMINISTRATIVO", "ADMIN", "SUPER_USUARIO"]), async (req, res) => {
    try {
        const todosPuntos = await prisma.puntoAtencion.findMany({
            where: { activo: true },
            orderBy: [{ es_principal: "desc" }, { nombre: "asc" }],
        });
        const formatted = todosPuntos.map((p) => formatPoint(p));
...
router.post("/", authenticateToken, requireRole(["ADMIN", "SUPER_USUARIO"]), async (req, res) => {
    try {
        const { nombre, direccion, ciudad, provincia, codigo_postal, telefono, servientrega_agencia_codigo, servientrega_agencia_nombre, } = req.body;
...
router.put("/:id", authenticateToken, requireRole(["ADMIN", "SUPER_USUARIO"]), async (req, res) => {
    try {
        ...
        const { nombre, direccion, ciudad, provincia, codigo_postal, telefono, activo, servientrega_agencia_codigo, servientrega_agencia_nombre, } = req.body;
        ...
        res.status(200).json({
            point: formatPoint(updatedPoint),
            success: true,
            timestamp: new Date().toISOString(),
        });
    }
...
router.patch("/:id/toggle", authenticateToken, requireRole(["ADMIN", "SUPER_USUARIO"]), async (req, res) => {
    try {
        const pointId = req.params.id;
        const existingPoint = await prisma.puntoAtencion.findUnique({
            where: { id: pointId },
...
router.get("/active-for-transfers", authenticateToken, async (req, res) => {
    try {
        const puntosActivos = await prisma.puntoAtencion.findMany({
            where: { activo: true },
            orderBy: [{ es_principal: "desc" }, { nombre: "asc" }],
        });
        const formatted = puntosActivos.map((p) => formatPoint(p));
...
router.get("/for-balance-management", authenticateToken, requireRole(["ADMIN", "SUPER_USUARIO"]), async (req, res) => {
    try {
        const puntosParaSaldos = await prisma.puntoAtencion.findMany({
            where: { activo: true },
            orderBy: [{ es_principal: "desc" }, { nombre: "asc" }],
        });
        const formatted = puntosParaSaldos.map((p) => formatPoint(p));
