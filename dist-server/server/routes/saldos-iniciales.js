...
router.post("/", authenticateToken, requireRole(["ADMIN", "SUPER_USUARIO"]), async (req, res) => {
    ...
    try {
        const { punto_atencion_id, moneda_id, cantidad_inicial, observaciones, billetes, monedas_fisicas, } = req.body ?? {};
        // Validaciones base: aceptar cantidad_inicial o billetes/monedas_fisicas
        const hasCantidad = cantidad_inicial !== undefined && cantidad_inicial !== null;
        const hasDesglose = billetes !== undefined || monedas_fisicas !== undefined;
        ...
        const billetesNum = billetes !== undefined ? toNumber(billetes) : undefined;
        const monedasNum = monedas_fisicas !== undefined ? toNumber(monedas_fisicas) : undefined;
        if ((billetes !== undefined &&
        ...
        const resultado = await prisma.$transaction(async (tx) => {
            ...
            if (existingInicial) {
                const baseInicial = new Prisma.Decimal(existingInicial.cantidad_inicial ?? 0);
                saldoInicialResult = await tx.saldoInicial.update({
                    where: { id: existingInicial.id },
                    data: {
                        cantidad_inicial: baseInicial.add(decCantidad),
                        observaciones: observaciones ?? existingInicial.observaciones ?? null,
                    },
                });
            }
            else {
            ...
            let saldoResult;
            if (existingSaldo) {
                const baseCantidad = new Prisma.Decimal(existingSaldo.cantidad ?? 0).add(decCantidad);
                const baseBilletes = new Prisma.Decimal(existingSaldo.billetes ?? 0).add(decBilletes);
                const baseMonedas = new Prisma.Decimal(existingSaldo.monedas_fisicas ?? 0).add(decMonedas);
