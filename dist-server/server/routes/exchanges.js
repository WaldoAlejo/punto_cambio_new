...
import { EstadoTransaccion, TipoOperacion, TipoViaTransferencia, } from "@prisma/client";
...
const isUSDByCode = (codigo) => (codigo || "").toUpperCase() === "USD";
const round2 = (x) => Math.round((Number(x) + Number.EPSILON) * 100) / 100;
async function getSaldo(tx, puntoId, monedaId) {
...
async function upsertSaldoEfectivoYBancos(tx, puntoId, monedaId, data) {
        ...
        return tx.saldo.update({
            ...
            data: {
                ...
                ...(typeof existing.bancos !== "undefined"
                    ? {
                        bancos: typeof data.bancos === "number"
                            ? round2(data.bancos)
                            : existing.bancos,
                    }
                    : {}),
...
async function logMovimientoSaldo(tx, data) {
    // Usar servicio centralizado
    const tipoMov = data.tipo_movimiento === "INGRESO"
        ? TipoMovimiento.INGRESO
        : data.tipo_movimiento === "EGRESO"
            ? TipoMovimiento.EGRESO
            : TipoMovimiento.AJUSTE;
    const tipoRef = data.tipo_referencia === "CAMBIO_DIVISA"
...
async (req, res) => {
    try {
        ...
        let { moneda_origen_id, moneda_destino_id, monto_origen, monto_destino, tasa_cambio_billetes, tasa_cambio_monedas, tipo_operacion, punto_atencion_id, datos_cliente, divisas_entregadas_billetes, divisas_entregadas_monedas, divisas_entregadas_total, divisas_recibidas_billetes, divisas_recibidas_monedas, divisas_recibidas_total, observacion, metodo_entrega, usd_entregado_efectivo, usd_entregado_transfer, metodo_pago_origen, usd_recibido_efectivo, usd_recibido_transfer, transferencia_numero, transferencia_banco, transferencia_imagen_url, abono_inicial_monto, abono_inicial_fecha, abono_inicial_recibido_por, saldo_pendiente, referencia_cambio_principal, } = req.body;
        ...
        const tasaEfectiva = num(tasa_cambio_billetes) > 0
        ...
        function getRateModeForPair(codOrigen, codDestino) {
        ...
        function convertir(_tipo, modo, montoOrigen, tasa, codOrigen, codDestino) {
        ...
        const entregadas_total_calc = num(divisas_entregadas_billetes) + num(divisas_entregadas_monedas);
        const recibidas_total_calc = num(divisas_recibidas_billetes) + num(divisas_recibidas_monedas);
        let monto_origen_final = num(monto_origen) > 0 ? num(monto_origen) : entregadas_total_calc;
        let monto_destino_final = num(monto_destino) > 0 ? num(monto_destino) : recibidas_total_calc;
        const divisas_entregadas_total_final = num(divisas_entregadas_total) > 0
            ? num(divisas_entregadas_total)
            : entregadas_total_calc;
        const divisas_recibidas_total_final = num(divisas_recibidas_total) > 0
        ...
        if (tasaEfectiva > 0 &&
            ...
            if (monto_destino_final === 0 && monto_origen_final > 0) {
                const { montoDestinoCalc } = convertir(tipo_operacion, modo, monto_origen_final, tasaEfectiva, codigoOrigen, codigoDestino);
        ...
        if (isUSDByCode(monedaDestino.codigo)) {
            if (metodo_entrega === "efectivo") {
                ...
                if (round2((usd_entregado_efectivo || 0) + (usd_entregado_transfer || 0)) !== round2(tot)) {
                    // fallback prudente
        ...
        if (metodo_pago_origen === TipoViaTransferencia.EFECTIVO) {
            ...
            if (round2((usd_recibido_efectivo || 0) + (usd_recibido_transfer || 0)) !== round2(tot)) {
        ...
        const exchange = await prisma.$transaction(async (tx) => {
            ...
            const cambio = await tx.cambioDivisa.create({
                data: {
                    ...
                    metodo_entrega,
                    transferencia_numero: metodo_entrega !== "efectivo"
                        ? transferencia_numero || null
                        : null,
                    transferencia_banco: metodo_entrega !== "efectivo"
                        ? transferencia_banco || null
                        : null,
                    transferencia_imagen_url: metodo_entrega !== "efectivo"
                        ? transferencia_imagen_url || null
                        : null,
                    // DESTINO USD
                    usd_entregado_efectivo: typeof usd_entregado_efectivo === "number"
                        ? round2(usd_entregado_efectivo)
                        : null,
                    usd_entregado_transfer: typeof usd_entregado_transfer === "number"
                        ? round2(usd_entregado_transfer)
                        : null,
                    // ORIGEN (nuevo)
                    metodo_pago_origen,
                    usd_recibido_efectivo: typeof usd_recibido_efectivo === "number"
                        ? round2(usd_recibido_efectivo)
                        : null,
                    usd_recibido_transfer: typeof usd_recibido_transfer === "number"
                        ? round2(usd_recibido_transfer)
                        : null,
                    abono_inicial_monto: typeof abono_inicial_monto === "number"
                    ...
                    saldo_pendiente: typeof saldo_pendiente === "number"
                    ...
                    divisas_entregadas_billetes: round2(num(divisas_entregadas_billetes)),
            ...
            const saldoOrigen = await getSaldo(tx, punto_atencion_id, moneda_origen_id);
            ...
            const origenAnteriorBk = typeof saldoOrigen?.bancos !== "undefined"
            ...
            const ingresoBil = ingresoEf > 0 ? round2(num(divisas_entregadas_billetes)) : 0;
            const ingresoMon = ingresoEf > 0 ? round2(num(divisas_entregadas_monedas)) : 0;
            ...
            const saldoDestino = await getSaldo(tx, punto_atencion_id, moneda_destino_id);
            ...
            const destinoAnteriorBk = typeof saldoDestino?.bancos !== "undefined"
            ...
            if (destinoAnteriorEf < egresoEf) {
                throw new Error("Saldo efectivo insuficiente en moneda destino para realizar el cambio");
            }
            if (typeof saldoDestino?.bancos !== "undefined" &&
                destinoAnteriorBk < egresoBk) {
                throw new Error("Saldo en bancos insuficiente para realizar el cambio por transferencia");
            }
            ...
            const billetesEgreso = egresoEf > 0 ? round2(num(divisas_recibidas_billetes)) : 0;
            const monedasEgreso = egresoEf > 0 ? round2(num(divisas_recibidas_monedas)) : 0;
            const destinoNuevoBil = Math.max(0, round2(destinoAnteriorBil - billetesEgreso));
            const destinoNuevoMon = Math.max(0, round2(destinoAnteriorMon - monedasEgreso));
            ...
            if (egresoBk > 0 && typeof saldoDestino?.bancos !== "undefined") {
                await logMovimientoSaldo(tx, {
                    ...
                    referencia_id: cambio.id,
                    tipo_referencia: "CAMBIO_DIVISA",
                    descripcion: `Egreso por cambio (bancos, destino) ${transferencia_banco || ""} ${transferencia_numero || ""}`.trim(),
                });
            }
            return cambio;
        });
...
router.get("/", authenticateToken, async (req, res) => {
    try {
        ...
        if (estadoParam === EstadoTransaccion.PENDIENTE ||
        ...
        try {
            ...
            logger.warn("Error con relaciones, usando consulta básica", {
                error: relationError instanceof Error ? relationError.message : "Unknown",
            });
...
router.patch("/:id/completar", authenticateToken, async (req, res) => {
    const { id } = req.params;
    const { metodo_entrega, transferencia_numero, transferencia_banco, transferencia_imagen_url, divisas_recibidas_billetes, divisas_recibidas_monedas, divisas_recibidas_total, } = req.body || {};
    try {
        if (!req.user?.id) {
            res
                .status(401)
        ...
        const updated = await prisma.cambioDivisa.update({
            ...
            data: {
                ...
                transferencia_numero: (metodo_entrega || cambio.metodo_entrega) === "transferencia"
                    ? transferencia_numero || cambio.transferencia_numero
                    : null,
                transferencia_banco: (metodo_entrega || cambio.metodo_entrega) === "transferencia"
                    ? transferencia_banco || cambio.transferencia_banco
                    : null,
                transferencia_imagen_url: (metodo_entrega || cambio.metodo_entrega) === "transferencia"
                    ? transferencia_imagen_url || cambio.transferencia_imagen_url
                    : null,
                divisas_recibidas_billetes: typeof divisas_recibidas_billetes === "number"
                    ? round2(divisas_recibidas_billetes)
                    : cambio.divisas_recibidas_billetes,
                divisas_recibidas_monedas: typeof divisas_recibidas_monedas === "number"
                    ? round2(divisas_recibidas_monedas)
                    : cambio.divisas_recibidas_monedas,
                divisas_recibidas_total: typeof divisas_recibidas_total === "number"
...
router.get("/partial", authenticateToken, async (req, res) => {
    try {
        const { pointId } = req.query;
        const isAdmin = req.user?.rol === "ADMIN" || req.user?.rol === "SUPER_USUARIO";
        const where = {
            saldo_pendiente: { gt: 0 },
            estado: EstadoTransaccion.PENDIENTE,
        };
...
router.patch("/:id/complete-partial", authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const isAdmin = req.user?.rol === "ADMIN" || req.user?.rol === "SUPER_USUARIO";
        if (!isAdmin) {
...
router.patch("/:id/register-partial-payment", authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const { abono_inicial_monto, abono_inicial_fecha, observacion_parcial } = req.body;
        const exchange = await prisma.cambioDivisa.findUnique({
            where: { id },
            select: {
                id: true,
        ...
        res.json({
            success: true,
            exchange: updated,
            message: `Abono registrado. Saldo pendiente: ${saldoPendiente.toFixed(2)}`,
        });
    }
    catch (error) {
...
router.get("/search-customers", authenticateToken, async (req, res) => {
    try {
        ...
        const clientesFromRecibos = recibos.map((r) => {
            const dc = r.datos_operacion?.datos_cliente || {};
            return {
                id: r.id,
                nombre: dc.nombre || "",
        ...
        const resultados = Array.from(map.values())
            ...
            .sort((a, b) => new Date(b.fecha_ultima_operacion).getTime() -
...
router.post("/:id/recontabilizar", authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        if (!req.user?.id) {
            res
        ...
        const isDestinoUSD = isUSDByCode(cambio.monedaDestino?.codigo);
        const totDestino = num(cambio.divisas_recibidas_total || cambio.monto_destino);
        ...
        const egresoBk = isDestinoUSD
            ...
            : cambio.metodo_entrega === "transferencia"
                ? totDestino
                : 0;
        const totOrigen = num(cambio.divisas_entregadas_total || cambio.monto_origen);
        const ingresoEf = cambio.metodo_pago_origen === "EFECTIVO"
            ? totOrigen
            : cambio.metodo_pago_origen === "MIXTO"
                ? num(cambio.usd_recibido_efectivo)
                : 0;
        const ingresoBk = cambio.metodo_pago_origen === "BANCO"
            ? totOrigen
            : cambio.metodo_pago_origen === "MIXTO"
        ...
        const baseUrl = process.env.INTERNAL_API_BASE_URL || "http://localhost:3001/api";
        const url = `${baseUrl}/movimientos-contables/procesar-cambio`;
        const response = await axios.post(url, { cambio_id: cambio.id, movimientos }, {
...
router.delete("/:id", authenticateToken, requireRole(["ADMIN", "SUPER_USUARIO"]), async (req, res) => {
    try {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            const anteriorBk = typeof saldoOrigen?.bancos !== "undefined"
                ? num(saldoOrigen?.bancos)
                : 0;
            ...
            const nuevoBil = Math.max(0, round2(num(saldoOrigen?.billetes) -
                (ingresoEf > 0 ? num(cambio.divisas_entregadas_billetes) : 0)));
            const nuevoMon = Math.max(0, round2(num(saldoOrigen?.monedas_fisicas) -
            ...
            if (ingresoEf > 0) {
                await logMovimientoSaldo(tx, {
                    ...
                    referencia_id: cambio.id,
                    tipo_referencia: "CAMBIO_DIVISA",
                    descripcion: `Reverso eliminación cambio (origen efectivo) #${cambio.numero_recibo || ""}`,
                });
            }
            if (ingresoBk > 0 && typeof saldoOrigen?.bancos !== "undefined") {
                await logMovimientoSaldo(tx, {
                    ...
                    referencia_id: cambio.id,
                    tipo_referencia: "CAMBIO_DIVISA",
                    descripcion: `Reverso eliminación cambio (origen bancos) #${cambio.numero_recibo || ""}`,
                });
            }
            ...
            const antBk = typeof saldoDestino?.bancos !== "undefined"
                ? num(saldoDestino?.bancos)
                : 0;
            const egEf = num(cambio.usd_entregado_efectivo, 0);
            const egBk = num(cambio.usd_entregado_transfer, 0);
            const sumDestTotal = num(cambio.divisas_recibidas_total || cambio.monto_destino);
            ...
            const sumarBilletes = egEf > 0 ? round2(num(cambio.divisas_recibidas_billetes)) : 0;
            const sumarMonedas = egEf > 0 ? round2(num(cambio.divisas_recibidas_monedas)) : 0;
            const nuevoBilDest = round2(num(saldoDestino?.billetes) + sumarBilletes);
            const nuevoMonDest = round2(num(saldoDestino?.monedas_fisicas) + sumarMonedas);
            ...
            await logMovimientoSaldo(tx, {
                ...
                referencia_id: cambio.id,
                tipo_referencia: "CAMBIO_DIVISA",
                descripcion: `Reverso eliminación cambio (destino efectivo) #${cambio.numero_recibo || ""}`,
            });
            ...
            if (egBk > 0 && typeof saldoDestino?.bancos !== "undefined") {
                await logMovimientoSaldo(tx, {
                    ...
                    referencia_id: cambio.id,
                    tipo_referencia: "CAMBIO_DIVISA",
                    descripcion: `Reverso eliminación cambio (destino bancos) #${cambio.numero_recibo || ""}`,
                });
            }
