...
async function executeAutoReconciliation(req, options) {
    try {
        ...
        if (pointId && currencyId) {
            ...
            const resultado = await saldoReconciliationService.reconciliarSaldo(pointId, currencyId, userId);
            if (resultado.corregido) {
                logger.warn("🔧 Auto-reconciliación detectó y corrigió inconsistencia", {
                    pointId,
                    currencyId,
            ...
            logger.debug("Auto-reconciliación omitida: no se pudieron extraer pointId/currencyId", {
                pointId,
                currencyId,
                path: req.path,
                method: req.method,
 ...
 */
export const transferAutoReconciliation = (req, res, next) => {
    // Solo ejecutar si la respuesta fue exitosa
    const originalSend = res.send;
    res.send = function (data) {
        // Ejecutar reconciliación solo si la operación fue exitosa
            ...
            setImmediate(async () => {
                try {
                    ...
                    if (destinoId && currencyId) {
                        logger.info("🔄 Ejecutando auto-reconciliación en DESTINO después de transferencia", {
                            pointId: destinoId,
                            currencyId,
                            userId,
                        });
                        const resultadoDestino = await saldoReconciliationService.reconciliarSaldo(destinoId, currencyId, userId);
                        if (resultadoDestino.corregido) {
                            logger.warn("🔧 Auto-reconciliación DESTINO detectó y corrigió inconsistencia", {
                                pointId: destinoId,
                                currencyId,
                    ...
                    if (origenId && currencyId) {
                        logger.info("🔄 Ejecutando auto-reconciliación en ORIGEN después de transferencia", {
                            pointId: origenId,
                            currencyId,
                            userId,
                        });
                        const resultadoOrigen = await saldoReconciliationService.reconciliarSaldo(origenId, currencyId, userId);
                        if (resultadoOrigen.corregido) {
                            logger.warn("🔧 Auto-reconciliación ORIGEN detectó y corrigió inconsistencia", {
                                pointId: origenId,
                                currencyId,
