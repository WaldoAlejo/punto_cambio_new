 ...
 */
export async function validarSaldoSuficiente(req, res, next) {
    try {
        const { body } = req;
        ...
        for (const validacion of validaciones) {
            const saldoActual = await obtenerSaldoActual(validacion.puntoAtencionId, validacion.monedaId);
            if (saldoActual < validacion.montoRequerido) {
                return res.status(400).json({
                    error: "SALDO_INSUFICIENTE",
                    message: `Saldo insuficiente en ${validacion.puntoNombre}. Saldo actual: $${saldoActual.toFixed(2)} ${validacion.monedaCodigo}, requerido: $${validacion.montoRequerido.toFixed(2)}`,
                    details: {
 ...
 */
function esOperacionEgreso(body) {
    // Casos de egreso directo
    if (body.tipo_movimiento &&
        ["EGRESO", "RETIRO", "PAGO", "TRANSFERENCIA_SALIDA"].includes(body.tipo_movimiento)) {
        return true;
    }
 ...
 */
async function obtenerValidacionesRequeridas(body) {
    const validaciones = [];
    // Egreso directo
 ...
 */
async function obtenerSaldoActual(puntoAtencionId, monedaId) {
