...
export const reportDataService = {
    async getExchangesData(startDate, endDate) {
        ...
        const exchangesByPoint = exchanges.reduce((acc, exchange) => {
    ...
    async getTransfersData(startDate, endDate) {
        ...
        const transfersByPoint = transfers.reduce((acc, transfer) => {
    ...
    async getBalancesData() {
        ...
        const balancesByPoint = balances.reduce((acc, balance) => {
    ...
    async getUserActivityData(startDate, endDate) {
        ...
        const activityByPoint = userActivity.reduce((acc, activity) => {
    ...
    async getWorkTimeData(startDate, endDate, filters) {
        ...
        const salidasPorUsuarioYDia = new Map();
        for (const s of salidas) {
            ...
            const mins = s.duracion_minutos ??
                (s.fecha_regreso
                    ? Math.max(0, Math.round((s.fecha_regreso.getTime() - s.fecha_salida.getTime()) / 60000))
                    : 0);
            salidasPorUsuarioYDia.set(key, (salidasPorUsuarioYDia.get(key) || 0) + mins);
        }
        const results = jornadas.map((j) => {
            ...
            const totalMin = Math.max(0, Math.round((salida.getTime() - entrada.getTime()) / 60000));
            ...
            const lunchMin = j.fecha_almuerzo && j.fecha_regreso
                ? Math.max(0, Math.round((new Date(j.fecha_regreso).getTime() -
    ...
    async getExchangesDetailedData(startDate, endDate, filters) {
        const where = {
        ...
        const tasasPorGrupo = new Map();
        function keyMid(date, pointId, currencyPair) {
        ...
        const result = rows.map((r) => {
            ...
            const tasaMid = midAgg && midAgg.count > 0
            ...
            return {
                ...
                spread: spread,
                margen_bruto: margen,
                fuente_tasa_mid: midAgg && midAgg.count > 0 ? "promedio_operaciones" : "auto",
    ...
    async getTransfersDetailedData(startDate, endDate, filters) {
        const where = {
            fecha: { gte: startDate, lt: endDate },
            ...(filters?.estado ? { estado: filters.estado } : {}),
        ...
        return rows.map((r) => ({
            ...
            usuario_solicitante: r.usuarioSolicitante?.nombre || "Usuario desconocido",
    ...
    async getAccountingMovementsData(startDate, endDate, filters) {
        const where = {
            fecha: { gte: startDate, lt: endDate },
            ...(filters?.pointId ? { punto_atencion_id: filters.pointId } : {}),
            ...(filters?.userId ? { usuario_id: filters.userId } : {}),
        ...
        if (refIds.length > 0) {
            ...
            recibosMap = new Map(recibos.map((r) => [r.referencia_id, r.numero_recibo]));
        }
        return rows.map((r) => ({
            id: r.id,
            fecha: r.fecha.toISOString(),
    ...
    async getEodBalancesData(startDate, endDate, filters) {
        const where = {
            fecha: { gte: startDate, lt: endDate },
            ...(filters?.pointId ? { punto_atencion_id: filters.pointId } : {}),
        };
    ...
    async getPointAssignmentsData(startDate, endDate, filters) {
        const where = {
