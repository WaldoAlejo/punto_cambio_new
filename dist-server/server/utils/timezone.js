...
export function toGyeClock(date) {
...
export function fromGyeClockToUtc(date) {
    return new Date(date.getTime() + GYE_OFFSET_MS);
}
...
export function gyeDayRangeUtcFromDate(date) {
...
export function todayGyeDateOnly(now = new Date()) {
...
export function gyeParseDateOnly(dateStr) {
    const m = dateStr.match(/^(\d{4})-(\d{2})-(\d{2})$/);
    if (!m)
        throw new Error(`Fecha inválida (se espera YYYY-MM-DD): ${dateStr}`);
    const y = Number(m[1]);
    const mm = Number(m[2]);
    const dd = Number(m[3]);
    return { y, m: mm, d: dd };
...
export function gyeDayRangeUtcFromDateOnly(dateStr) {
...
export function gyeDateOnlyToUtcMidnight(dateStr) {
    const { y, m, d } = gyeParseDateOnly(dateStr);
    const startGyeAsUtc = new Date(Date.UTC(y, m - 1, d, 0, 0, 0, 0));
    return fromGyeClockToUtc(startGyeAsUtc);
}
// Implementación
export function gyeDayRangeUtcFromYMD(a, b, c) {
    if (typeof a === "string") {
        // "YYYY-MM-DD"
 ...
 */
export function toEcuadorTime(utcDate) {
 ...
 */
export function formatEcuadorDateTime(utcDate) {
 ...
 * Ejemplo: 03/10/2025
 */
export function formatEcuadorDate(utcDate) {
    const ecuadorDate = toEcuadorTime(utcDate);
    const day = String(ecuadorDate.getUTCDate()).padStart(2, "0");
    const month = String(ecuadorDate.getUTCMonth() + 1).padStart(2, "0");
    const year = ecuadorDate.getUTCFullYear();
 ...
 * Ejemplo: 18:30:45
 */
export function formatEcuadorTime(utcDate) {
    const ecuadorDate = toEcuadorTime(utcDate);
    const hours = String(ecuadorDate.getUTCHours()).padStart(2, "0");
    const minutes = String(ecuadorDate.getUTCMinutes()).padStart(2, "0");
 ...
 */
export function fromEcuadorToUTC(ecuadorDate) {
 ...
 */
export function createEcuadorDate(year, month, day, hour = 0, minute = 0, second = 0) {
    // Crear fecha como si fuera UTC
    const ecuadorAsUtc = new Date(Date.UTC(year, month - 1, day, hour, minute, second, 0));
